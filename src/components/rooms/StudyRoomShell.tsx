...
export default function StudyRoomShell({
  ...
  // Load existing messages from DB when conversationId is available
  useEffect(() => {
    if (!conversationId) return;

    // 1. Initial Load
    import('@/services/study-rooms.service').then(({ getRoomMessages }) => {
      getRoomMessages(conversationId).then(msgs => {
        if (msgs.length === 0) return;
        const mapped = msgs.map((m: any) => ({
          id: m.id,
          type: m.sender === 'assistant' ? 'ai' as const : 'user' as const,
          title: m.sender === 'assistant' ? 'RAYA Host' : 'Member', // title is dynamic here
          body: m.text,
          meta: m.sender === 'assistant' ? 'Moderator' : 'Earlier',
        }));
        ...
        (payload) => {
          ...
          setRoomMessages((prev) => {
            ...
            const mapped: RoomEvent = {
              ...
              title: newMessage.sender === 'assistant' ? 'RAYA Host' : 'Teammate',
