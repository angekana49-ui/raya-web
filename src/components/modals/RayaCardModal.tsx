  ...
  useEffect(() => {
    if (isOpen) {
      ...
      if (defaultTab === "backup") {
        ...
        // ── INSTANT: Generate or retrieve key synchronously ──
        let key = "";
        try {
        ...
        if (!key) {
          key = generateMasterKey(); // crypto.getRandomValues = synchronous, instant
          try { localStorage.setItem("raya_master_key", key); } catch { /* ignore */ }
        }

        // Key ready IMMEDIATELY — no async wait
        setMasterKey(key);

        // ── Server link in background (non-blocking) ──
        if (!upgradeSuccess) {
          linkKeyInBackground(key);
        }
      }
    }
            ...
            <div className="p-6 pt-4 flex-1 overflow-y-auto">
              {activeTab === "backup" && (
                <div className="space-y-6">
                  {viewsLeft === 0 ? (
                    /* No views left — key permanently hidden */
                    ...
                    <>
                      ...
                      {upgradeSuccess && !upgradeError && !isRevealed && (
